...
function create_environment {
	...
	for VARIABLE in ENV_YAML ENV_VAR_VALUES ENV_VARS USE_DB DB_CONTAINER \
			RANCHER_STACK RANCHER_STACK_ID RANCHER_SECRET_KEY \
			RANCHER_ACCESS_KEY RANCHER_API_URL RANCHER_SERVICE_NAME DOCKER_TAG_PREFIX \
			DOCKER_REPOSITORY_USER DOCKER_REPOSITORY_NAME PROJECT_NAME ; do
		#eval "local ${VARIABLE}_SED=$(echo \$VARIABLE | sed -e 's/[\/&]/\\&/g')"
		eval "local ${VARIABLE}_SED=\"\$(echo \$$VARIABLE | sed -e 's/[\/&]/\\&/g')\""
	done
	...
	for FILE in config.cfg service.yml.tpl scale.yml.tpl ; do
		...
		sed \
			...
			-e "s/%DOCKER_REPOSITORY_NAME%/$DOCKER_REPOSITORY_NAME_SED/g" \
			-e "s/%DOCKER_REPOSITORY_USER%/$DOCKER_REPOSITORY_USER_SED/g" \
			-e "s/%DOCKER_TAG_PREFIX%/$DOCKER_TAG_PREFIX_SED/g" \
			-e "s/%RANCHER_SERVICE_NAME%/$RANCHER_SERVICE_NAME_SED/g" \
			-e "s/%RANCHER_API_URL%/$RANCHER_API_URL_SED/g" \
			-e "s/%RANCHER_ACCESS_KEY%/$RANCHER_ACCESS_KEY_SED/g" \
			-e "s/%RANCHER_SECRET_KEY%/$RANCHER_SECRET_KEY_SED/g" \
			-e "s/%RANCHER_STACK_ID%/$RANCHER_STACK_ID_SED/g" \
			-e "s/%RANCHER_STACK%/$RANCHER_STACK_SED/g" \
			-e "s/%DB_CONTAINER%/$DB_CONTAINER_SED/g" \
			-e "s/%USE_DB%/$USE_DB_SED/g" \
			-e "s/%ENVIRONMENT_VARIABLES%/$ENV_VARS_SED/g" \
			-e "s/%ENVIRONMENT_VALUES%/$ENV_VAR_VALUES_SED/g" \
			-e "s/%ENVIRONMENT_DATA%/$ENV_YAML_SED/g" \
